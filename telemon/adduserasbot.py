...
client = TelegramClient(phone, api_id, api_hash).start(bot_token=bot_token)
client.connect()
...
async def tesadd(target_group_entity, user_to_add):
    a = await client(InviteToChannelRequest(target_group_entity,[user_to_add]))
    return a
