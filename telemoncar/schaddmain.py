...
def kick_user_from_group(service, userId, userHash):
    target_group_entity = ''
    # if service == "Football":
    ...
    # user_to_add = InputPeerUser(userId, userHash)
    # try:
    #     user_client.kick_participant(target_group_entity, user_to_add)
    #     print('user removed')
    # except:
    ...
    if data:
        for row in data:
            ...
            if today - user_date > timedelta(days=30):
                print(row)
                kick_user_from_group(row["service"], row["Userid"], row['userhash'])
    else:
        pass
...
async def pay_service_1(event):
    ...
    insertRow = [sender.id, sender.access_hash, "Football", "complete", str(date.today())]
    num_rows_filled += 1
